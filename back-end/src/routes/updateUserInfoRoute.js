...
export const updateUserInfoRoute = {
    ...
    handler: async (req, res) => {
        try {
            ...
            if (!isVerified) return res.status(403).json({message: 'Account must be verified before updating.'})
            ...
            const { email, isVerified: isVerifiedValue, info } = updateUser.value;

            const newToken = jwt.sign({ id, email,  isVerified: isVerifiedValue, info }, process.env.JWT_TOKEN, { expiresIn: '2d' });
